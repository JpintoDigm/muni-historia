...
import { MontserratLightItalic, MontserratMediumItalic, muni, MarcellusRegular } from "@/app/fonts/fonts";
import DesafiosCiudad from "../components/visionCiudad/DesafiosCiudad";
import SecondSection from "../components/visionCiudad/SecondSection";
...
function TypewriterTitle({
  ...
  return (
    <span>
      {render()}
      {cursor && !done && (
        <span className={`inline-block ml-1 animate-pulse ${cursorClassName}`}>|</span>
      )}
    </span>
  );
}
    ...
    <main className="min-h-screen">
      ...
      <section className="w-full px-4 py-0 md:py-8 bg-muni-azul">
        <div
          ref={heroRef}
          className={[
            "w-full container mx-auto flex flex-col gap-4 p-6 md:p-0",
        ...
        >
          <div className="flex flex-col-reverse md:flex-row justify-end items-center gap-3">
            <div className="flex items-center">
              <img
                src={`${basePath}/img/logo250blanco.svg`}
                alt="Icono 250"
          ...
          <div className="w-full sm:w-2/3">
            {/* TÍTULO con efecto letra por letra */}
            <h1
              className={`${MontserratMediumItalic.className} text-muni-verde text-justify text-4xl sm:text-7xl mb-4 font-bold leading-tight`}
            >
              <TypewriterTitle
                speed={26}
                cursorClassName="text-white"
                parts={[
                  { text: "La Visión", className: "" },
                  { text: "\n", className: "" },
                  { text: "de la ", className: "" },
                  { text: "Ciudad", className: "text-white" },
                ]}
              />
            </h1>
            ...
            <p
              className={[
                "text-white text-sm lg:text-xl text-justify mt-10",
                "transition-all duration-700 ease-out delay-150",
                heroIn ? "opacity-100 translate-y-0" : "opacity-0 translate-y-3",
              ].join(" ")}
            >
              La Visión de Ciudad define cómo{" "}
              <span className={`${MontserratMediumItalic.className} text-muni-verde font-bold`}>
                la Ciudad de Guatemala avanza día a día.
              </span>{" "}
              Es el plan de acción del alcalde Ricardo Quiñónez, que organiza el trabajo municipal, da continuidad a los
              proyectos y asegura{" "}
              <span className={`${MontserratMediumItalic.className} text-muni-verde font-bold`}>
            ...
            <p
              className={[
                "text-white text-sm lg:text-xl mt-5 md:mt-10 text-justify",
                "transition-all duration-700 ease-out delay-200",
                heroIn ? "opacity-100 translate-y-0" : "opacity-0 translate-y-3",
              ].join(" ")}
            >
              Este plan se desarrolla a través de ejes estratégicos que orientan cada acción:{" "}
              <span className={`${MontserratMediumItalic.className} text-muni-rosado font-bold`}>
                un lugar de oportunidades
              </span>
              , que impulsa el talento, el aprendizaje y el emprendimiento;{" "}
              <span className={`${MontserratMediumItalic.className} text-muni-amarillo font-bold`}>
                un lugar que inspira
              </span>
              , generando orgullo y sentido de pertenencia; y{" "}
              <span className={`${MontserratMediumItalic.className} text-muni-pantone font-bold`}>
                un lugar que conecta
              </span>
              , fortaleciendo la movilidad y los servicios para facilitar la vida diaria de las personas
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="relative z-10 w-full container mx-auto p-6 lg:px-0 lg:py-6">
          ...
          <div className="w-full sm:w-2/3">
            ...
            <p className="text-sm lg:text-xl text-muni-azul text-justify mb-10">
              La estrategia atiende los desafíos de la ciudad y potencia las oportunidades actuales y futuras para mejorar
              la calidad de vida y consolidar el modelo de la ciudad compacta y de proximidad.
            </p>
          </div>
      ...
      <section className="relative w-full px-4 py-6 bg-muni-azul overflow-hidden">
        ...
        <div className="my-10">
          <p className={`text-center font-bold mt-15 md:mb-100 text-xl md:text-4xl text-muni-verde px-0 md:px-80 italic ${MarcellusRegular.className}`}>
